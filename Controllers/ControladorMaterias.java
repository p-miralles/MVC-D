package Controllers;
import Model.Cursado;
import Model.Materia;
import Model.Profesor;
import Views.AgregaAP;
import Views.AgregaCMC;
import Views.MenuAlumnoProfesor;
import Views.MenuCMC;
import java.awt.event.ActionEvent;
import java.awt.event.ActionListener;
import java.sql.Date;
import java.text.SimpleDateFormat;
import java.util.List;
import javax.swing.JOptionPane;
import javax.swing.JTable;
import javax.swing.table.DefaultTableModel;
/**
 *
 * @author User-PC
 */
public class ControladorMaterias implements ActionListener {

    //Vistas
    public MenuCMC menu;
    AgregaCMC agrega = new AgregaCMC();
    public Materia materia = new Materia(); //Modelo
    //Controladores
     
    private String StatusFlag = "";

    public ControladorMaterias(MenuCMC menu, AgregaCMC agrega) {
        this.menu = menu;
        this.menu.jbAdd.addActionListener(this);
        this.menu.jbDel.addActionListener(this);
        this.menu.jbEdit.addActionListener(this);
        this.menu.jbClose.addActionListener(this);
        
        this.agrega.jbAdd.addActionListener(this);
        this.agrega.jbCancel.addActionListener(this);
    }

    public ControladorMaterias() {
    }



    public void iniciar() {
        menu.setTitle("Menu Materias");
        menu.setLocationRelativeTo(null);
        menu.menuTitulo.setText("Gestión de Materias");
        listar();
    }

    public void actionPerformed(ActionEvent e) {

        if (e.getSource().equals(menu.jbAdd)) {
            System.out.println("Menu Add Materia presionado");
            StatusFlag = "adding";            
            menu.setEnabled(false);
            menu.setLocationRelativeTo(null);
            agrega.CreaUpdTitulo.setText("Nueva Carrera");
            agrega.jbAdd.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Images/add.png")));
            agrega.jlA.setText("Código");
            agrega.jlB.setText("Nombre");
            agrega.jlC.setText("DNI Profesor");
            //ctrlAlumnos.iniciar();
            agrega.setVisible(true);

        } 
        
        if(e.getSource().equals(menu.jbDel)){
            eliminar();
        }
        
        if(e.getSource().equals(menu.jbEdit)){
            System.out.println("Menu Editar Materia presionado");            
            preparaEdicion();            
        }
        
        if(e.getSource().equals(menu.jbClose)){
            menu.setVisible(false);
        }        
        
        
        if(e.getSource().equals(agrega.jbCancel)){
            agrega.setVisible(false);
            menu.setEnabled(true);
            agrega.jbA.setText("");
            agrega.jbB.setText("");
            agrega.jbC.setText("");
        }
        if(e.getSource().equals(agrega.jbAdd)){
            System.out.println("Boton ADD/OK");
            if(StatusFlag=="adding") agregar();
            else{
                if(StatusFlag=="editing") editar();
                else System.out.println("log: error de lógica de botones!!");
            }
        }
        
        
        
    }//end action performed
    
    
    public void listar() {
        List<Materia> matList = materia.readMaterias();
        //panelAlumnos.getTblAlumnos().setRowHeight(30);
        String column[]={"Codigo","Nombre","DNI Profesor"};  
        String data[][]={};
        DefaultTableModel dtm= new DefaultTableModel(data,column); 

        for (int i = 0; i < matList.size(); i++) {
            dtm.addRow(new Object[] { 
                    String.valueOf(matList.get(i).getCodigo()),
                    matList.get(i).getNombre(),
                    matList.get(i).getDniProfesor()
            });
        }
        menu.jTable1.setModel(dtm);
    }
    

    public void agregar() {
        Profesor profesor = new Profesor();
        //to do: agregar más validaciones a todos los campos
        if (validarVacios() == false) {
            JOptionPane.showMessageDialog(null, "Ningun campo debe quedar vacío!!");
        } else if (materia.materiaExist(Integer.valueOf(agrega.jbA.getText())) == true) {
            JOptionPane.showMessageDialog(null, "El código de materia que intenta agregar ya existe!");
        } else if (profesor.findProfesor(Integer.valueOf(agrega.jbC.getText())).getDni() == 0) {
            JOptionPane.showMessageDialog(null, "Error: el DNI del profesor ingresado no existe.");
        }
        else {
            System.out.println("Intentando guardar...");
            //Se Crea El Alumno Una Vez Que Se Validaron Los Datos Ingresados Por La Vista
   
            materia.setCodigo(Integer.valueOf(agrega.jbA.getText()));
            materia.setNombre(agrega.jbB.getText());
            materia.setDniProfesor(Integer.valueOf(agrega.jbC.getText()));
            if (materia.createMateria(materia) == true) {
            agrega.setVisible(false);
            menu.setEnabled(true);
            agrega.jbA.setText("");
            agrega.jbB.setText("");
            agrega.jbC.setText("");
            
                listar();
                JOptionPane.showMessageDialog(null, "Guardado Con Exito!");

            } else {
                JOptionPane.showMessageDialog(null, "ERROR");

            }
        }
    }
    
    public void eliminar() {
        Cursado cursado = new Cursado();
        int row = menu.jTable1.getSelectedRow();
        if (row == -1)
            JOptionPane.showMessageDialog(null, "Debe Seleccione Una Fila!");
        else {
            int cod = Integer.parseInt((String) menu.jTable1.getValueAt(row, 0).toString());
              if (cursado.findMateria(cod).getCodMateria() != 0) {
                JOptionPane.showMessageDialog(null, "Error: La materia que intenta eliminar tiene cursados asociados.");
            } else
            if (JOptionPane.showConfirmDialog(null, "Eliminar Materia", "Desea Eliminar la materia?", JOptionPane.YES_NO_OPTION) == 0) {
                materia.deleteMateria(cod);
                listar();                
                JOptionPane.showMessageDialog(null, "Eliminada!");
            }

        }
    }

        public void preparaEdicion() {
        int row = menu.jTable1.getSelectedRow();
        if (row == -1)
            JOptionPane.showMessageDialog(null, "Debe Seleccione Una Fila!");
        else {
            StatusFlag = "editing";
            menu.setEnabled(false);
            menu.setLocationRelativeTo(null);
            agrega.CreaUpdTitulo.setText("Editar Carrera");
            //agrega.jbAdd.setIcon(new ImageIcon("Images//ok.png"));
            agrega.jbAdd.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Images/ok.png")));
            //cambiar nombres campos aqui
            agrega.setVisible(true);
            //carga los campos con los valores a editar
            int cod = Integer.valueOf(menu.jTable1.getValueAt(row, 0).toString());
            //cargo Alumno del DAO (DB)
            materia = materia.findMateria(cod);
            //Lleno el formulario
            agrega.jbA.setText(String.valueOf(materia.getCodigo()));
            agrega.jbB.setText(String.valueOf(materia.getNombre()));
            agrega.jbC.setText(String.valueOf(materia.getDniProfesor()));
            
    }
   }
//-----------------------------------------------
    public void editar(){
        if (validarVacios() == false) {
            JOptionPane.showMessageDialog(null, "Todos Los Campos Deben Estar Completos!");
        } else 
            //TODO: realizar validaciones extra
          {
            materia.setCodigo(Integer.valueOf(agrega.jbA.getText()));
            materia.setNombre(agrega.jbB.getText());
            materia.setDniProfesor(Integer.valueOf(agrega.jbC.getText()));
            
            if (materia.updateMateria(materia) == true) {
                listar();
                StatusFlag ="";
                agrega.setVisible(false);
                menu.setEnabled(true);
                agrega.jbA.setText("");
                agrega.jbB.setText("");
                agrega.jbC.setText("");
                JOptionPane.showMessageDialog(null, "Edición exitosa");
            }
            else {
               JOptionPane.showMessageDialog(null, "ERROR, Revisar Consola...");
            }
        }
    }
//-----------------------------------------------
    public boolean validarVacios() {
        if ( agrega.jbA.getText().isEmpty() || 
             agrega.jbB.getText().isEmpty() ||
             agrega.jbC.getText().isEmpty()
            )
             return false;
        else return true;
    }
//------------------------------------------------

}//end class